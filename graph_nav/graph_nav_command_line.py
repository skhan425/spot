...
class GraphNavInterface(object):
    ...
    def __init__(self, robot, upload_path, use_gps=False):
        ...
        self._current_waypoints = dict()  #maps id to waypoint
        ...
        self._command_dictionary = {
            ...
            '8': (self._navigate_spanning_route_with_action,
                  "Navigate route spanning all waypoints where an action is performed at each waypoint."),
            '9': (
            ...
            '10': (self._clear_graph, "Clear the current graph.")
        }
    ...
    def _navigate_spanning_route_with_action(self, *args):
        """Navigate through a route spanning every waypoint. For each waypoint, an action is performed.
        Works best if started from the source vertex (route will be spanning)."""
    ...
    def _get_waypoint_snapshot(self, id):
        """Get waypoint from graph (return None if waypoint not found)"""

        if self._current_graph is None:
            return

        for waypoint_snapshot in self._current_waypoint_snapshots:
